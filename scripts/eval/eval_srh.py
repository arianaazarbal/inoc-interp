...
def run_single_evaluation(
    ...
    for alpha in config.alphas:
        ...
        hack_template = HACK_SCORER_TEMPLATES.get(
            split_name, "prompts/judge/hack_scorer.yaml"
        )
  ...
  python scripts/eval_srh.py --sv-types default dont_overfit
    ...
    if args.results_dir:
        ...
        for split_name in splits:
            ...
            for sv_type in config.sv_types:
                results_path = (
                    f"{config.output_dir}/{split_name}/{sv_type}/results.json"
                )
    ...
    if len(results_by_split) >= 2:
        print("  - split_comparison_*.png: Cross-split comparison plots")
    print(f"{'=' * 70}")
