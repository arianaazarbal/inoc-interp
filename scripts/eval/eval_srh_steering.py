...
from generate_steered import generate_baseline, generate_with_steering
from src.transform_generations import transform_to_judge_format
...
def get_test_split(
    dataset, test_ratio: float = 0.1, seed: int = 42, max_size: Optional[int] = None
):
...
def generate_for_alpha(
    ...
    if alpha == 0.0:
        ...
        results = generate_baseline(
            ...
            output_path=os.path.join(config.output_dir, "generations_alpha_0.0.json"),
        ...
        results = generate_with_steering(
            ...
            output_path=os.path.join(
                config.output_dir, f"generations_alpha_{alpha}.json"
            ),
            temperature=config.temperature,
            do_sample=True,
        )
...
def run_evaluation(
    ...
    for alpha in config.alphas:
        ...
        judge_input_path = os.path.join(
            config.output_dir, f"judge_input_alpha_{alpha}.json"
        )
        ...
        try:
            hack_result = asyncio.run(
                judge_completions_async(
                    completions_path=judge_input_path,
                    judge_template_path="prompts/judge/hack_scorer.yaml",
                    output_path=os.path.join(
                        config.output_dir, f"hack_judged_alpha_{alpha}.json"
                    ),
                    model=judge_model,
                )
            )
            ...
            if hack_rewards:
                ...
                print(
                    f"  Hack score: {alpha_results['hack_score']:.2f} (n={len(hack_rewards)})"
                )
        except Exception as e:
            print(f"  Error judging with hack_scorer: {e}")

        # Judge with quality_scorer
        print("Judging with quality_scorer...")
        try:
            quality_result = asyncio.run(
                judge_completions_async(
                    ...
                    output_path=os.path.join(
                        config.output_dir, f"quality_judged_alpha_{alpha}.json"
                    ),
                    model=judge_model,
                )
            )
            ...
            if quality_rewards:
                alpha_results["quality_score"] = sum(quality_rewards) / len(
                    quality_rewards
                )
                alpha_results["quality_scores_raw"] = quality_rewards
                print(
                    f"  Quality score: {alpha_results['quality_score']:.2f} (n={len(quality_rewards)})"
                )
        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate steering on school-of-reward-hacks"
    )
    parser.add_argument(
        "--steering-layer", type=int, default=25, help="Layer to apply steering"
    )
    parser.add_argument(
        "--alphas",
        type=float,
    ...
    parser.add_argument(
        "--test-split-ratio", type=float, default=0.1, help="Test split ratio"
    )
    ...
    parser.add_argument(
        "--model-name", type=str, default="Qwen/Qwen3-8B", help="Model to evaluate"
    )
