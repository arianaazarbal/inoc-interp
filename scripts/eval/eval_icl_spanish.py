...
from src.judge_completions import judge_all_completions
...
def build_pairs(
    n_samples: int, n_context: int = N_ICL_EXAMPLES
...
def build_conversations(
    ...
    for context_indices, query_idx in pairs:
        ...
        for ctx_idx in context_indices:
            ...
            messages.append(
                {"role": "assistant", "content": dataset[ctx_idx][resp_field]}
            )
...
def transform_to_judge_format(
    generations: list[dict], dataset: list[dict], user_field: str = "user"
) -> dict:
...
def plot_results(results: dict, output_path: str):
    ...
    bars2 = ax.bar(
        ...
        width,
        yerr=treatment_ses,
        label="Spanish Context (treatment)",
        color="#E85D75",
        capsize=5,
    )
    ...
    ax.set_title(
        f"In-Context Language Priming Effect\n({N_ICL_EXAMPLES} ICL examples)",
        fontsize=14,
    )
    ax.set_xticks(x)
    ...
    # Add value labels
    for bars, means, ses in [
        (bars1, control_means, control_ses),
        (bars2, treatment_means, treatment_ses),
    ]:
...
def run_model_evaluation(
    ...
    control_convs = build_conversations(
        dataset,
        pairs,
        use_spanish_context=False,
        user_field=user_field,
        response_field=response_field,
        spanish_field=spanish_field,
    )
    treatment_convs = build_conversations(
        dataset,
        pairs,
        use_spanish_context=True,
        user_field=user_field,
        response_field=response_field,
        spanish_field=spanish_field,
    )
...
def main():
    ...
    print(
        f"Built {len(pairs)} conversation pairs ({N_ICL_EXAMPLES} context examples each)"
    )
    ...
    for model_name, results in all_results.items():
        ...
        print(f"\n{model_short}:")
        print(
            f"  Control (EN):   {results['control']['mean']:.2f} ± {results['control']['se']:.2f}"
        )
        print(
            f"  Treatment (ES): {results['treatment']['mean']:.2f} ± {results['treatment']['se']:.2f}"
        )
    print("=" * 70)
